...
float2 TerrainNodeInstance::get_offset_level(const float2& camera_pos, unsigned int level)
{
	if (level == 0) // Must follow level 1 as trim region is fixed.
	{
		...
		return get_offset_level(camera_pos, 1) + float2(temp_size, temp_size);
	}
	else
	{
		...
		float2 temp_value(1 << (level + 1), 1 << (level + 1));
		float2 vec_floor;
		...
		float2 pos = snapped_pos - float2((2 * (terrain_node->size - 1)) << level, (2 * (terrain_node->size - 1)) << level);
		return pos;
	}
}
//! [Snapping clipmap level to a grid]
...
void TerrainNodeInstance::update_level_offsets(const float2& camera_pos)
{
	...
	for (unsigned int i = 0; i < terrain_node->levels; i++){
...
TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_horiz_fixup(InstanceData *instances)
{
	DrawInfo info;
	...
	for (unsigned int i = 1; i < terrain_node->levels; i++)
	{
		...
		instance.offset = instance.offset + float2::multiply(float2(0, 2 * (terrain_node->size - 1)), float2(1 << i, 1 << i));
		...
		*instances++ = instance;
		...
		instance.offset = level_offsets[i];
		instance.offset = instance.offset + float2::multiply(float2(3 * (terrain_node->size - 1) + 2, 2 * (terrain_node->size - 1)), float2(1 << i, 1 << i));
		instance.offset = float2::multiply(instance.offset, float2(terrain_node->clipmap_scale, terrain_node->clipmap_scale));
...
TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_vert_fixup(InstanceData *instances)
{
	DrawInfo info;
	InstanceData instance;
	//instance.debug_color = Vector3(0, 1, 0);
	...
	for (unsigned int i = 1; i < terrain_node->levels; i++)
	{
		...
		instance.offset = level_offsets[i];
		instance.offset = instance.offset + float2::multiply(float2(2 * (terrain_node->size - 1), 0), float2(1 << i, 1 << i));
		instance.offset = float2::multiply(instance.offset, float2(terrain_node->clipmap_scale, terrain_node->clipmap_scale));
		...
		*instances++ = instance;
		...
		// Bottom region
		instance.offset = level_offsets[i];
		instance.offset = instance.offset + float2::multiply(float2(2 * (terrain_node->size - 1), 3 * (terrain_node->size - 1) + 2), float2(1 << i, 1 << i));
		instance.offset = float2::multiply(instance.offset, float2(terrain_node->clipmap_scale, terrain_node->clipmap_scale));
...
TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_degenerate(InstanceData *instances, const Block& block, const float2& offset, const float2& ring_offset)
{
	DrawInfo info;
	info.instances = 0;
	...
	for (unsigned int i = 0; i < terrain_node->levels - 1; i++)
	{
		instance.level = i;
		instance.offset = level_offsets[i];
		instance.offset = instance.offset + float2::multiply(offset, float2(1 << i, 1 << i));
		...
		if (i > 0){
			instance.offset = instance.offset + float2::multiply(ring_offset, float2(1 << i, 1 << i));
		}
		instance.offset = float2::multiply(instance.offset, float2(terrain_node->clipmap_scale, terrain_node->clipmap_scale));
		instance.scale = terrain_node->clipmap_scale * float(1 << i);
...
TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_degenerate_left(InstanceData *instances)
{
	return get_draw_info_degenerate(instances, terrain_node->degenerate_left, float2(0.0f, 0.0f), float2(0.0f, 0.0f));
}

TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_degenerate_right(InstanceData *instances)
{
	return get_draw_info_degenerate(instances, terrain_node->degenerate_right, float2(4 * (terrain_node->size - 1), 0.0f), float2(2.0f, 0.0f));
}

TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_degenerate_top(InstanceData *instances)
{
	return get_draw_info_degenerate(instances, terrain_node->degenerate_top, float2(0.0f, 0.0f), float2(0.0f, 0.0f));
}

TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_degenerate_bottom(InstanceData *instances)
{
	return get_draw_info_degenerate(instances, terrain_node->degenerate_bottom, float2(0.0f, 4 * (terrain_node->size - 1)), float2(0.0f, 2.0f));
}

// Only used for cliplevel 1 to encapsulate cliplevel 0.
TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_trim_full(InstanceData *instances)
{
	DrawInfo info;
	...
	instance.offset = instance.offset + float2((terrain_node->size - 1) << 1, (terrain_node->size - 1) << 1);
	instance.offset = float2::multiply(instance.offset, float2(terrain_node->clipmap_scale, terrain_node->clipmap_scale));
...
TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_trim(InstanceData *instances, const Block& block, TrimConditional cond)
{
	DrawInfo info;
	info.index_buffer_offset = block.offset;
	info.indices = block.count;
	...
	for (unsigned int i = 2; i < terrain_node->levels; i++)
	{
		float2 offset_prev_level = level_offsets[i - 1];
		float2 offset_current_level = level_offsets[i] + float2((terrain_node->size - 1) << i, (terrain_node->size - 1) << i);
		...
		instance.level = i;
		instance.offset = level_offsets[i];
		instance.offset = instance.offset + float2((terrain_node->size - 1) << i, (terrain_node->size - 1) << i);
...
TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_trim_top_right(InstanceData *instances)
{
	return get_draw_info_trim(instances, terrain_node->trim_top_right, trim_top_right_cond);
}

TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_trim_top_left(InstanceData *instances)
{
	return get_draw_info_trim(instances, terrain_node->trim_top_left, trim_top_left_cond);
}

TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_trim_bottom_right(InstanceData *instances)
{
	return get_draw_info_trim(instances, terrain_node->trim_bottom_right, trim_bottom_right_cond);
}

TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_trim_bottom_left(InstanceData *instances)
{
	return get_draw_info_trim(instances, terrain_node->trim_bottom_left, trim_bottom_left_cond);
}

// These are the basic N-by-N tesselated quads.
TerrainNodeInstance::DrawInfo TerrainNodeInstance::get_draw_info_blocks(InstanceData *instances)
{
	...
	for (unsigned int z = 0; z < 4; z++)
	{
		for (unsigned int x = 0; x < 4; x++)
		{
			...
			instance.offset = instance.offset + float2::multiply(float2(x, z), float2(terrain_node->size - 1, terrain_node->size - 1));
			instance.offset = float2::multiply(instance.offset, float2(terrain_node->clipmap_scale, terrain_node->clipmap_scale));
	...
	for (unsigned int i = 1; i < terrain_node->levels; i++)
	{
		for (unsigned int z = 0; z < 4; z++)
		{
			for (unsigned int x = 0; x < 4; x++)
			{
				...
				instance.level = i;
				instance.offset = level_offsets[i];
				instance.offset = instance.offset + float2::multiply(float2(x, z), float2((terrain_node->size - 1) << i, (terrain_node->size - 1) << i));
...
void TerrainNodeInstance::render_draw_list()
{
	...
	for (int i = 0; i < draw_list.Size(); i++)
	{
		if (draw_list[i].instances){
			...
			renderDevice->DrawIndexedInstanced(draw_list[i].instances, 0);
